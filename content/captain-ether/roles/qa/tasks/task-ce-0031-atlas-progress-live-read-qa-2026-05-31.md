# QA Task: TASK-CE-0031 Atlas Progress Live-Read QA
...
Review the legacy `progress` live-read cutover slice only.
...
2. live-read smoke passes with Atlas read enabled for legacy `progress`;
3. forced read failure falls back to JSON without breaking smoke;
4. no privacy regression appears in progress payloads;
5. no scope widening occurred into `captain_ether_stream_progress`,
   weak-points, watch-sessions, or auth.

## Next Expected Gate

Director acceptance of the legacy `progress` live-read slice.
